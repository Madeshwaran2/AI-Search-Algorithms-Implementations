...
def astar_search(graph, start, goal):
    ...
    path.append(start)
    path_cost = get_geographical_heuristic_heuristic(start, goal)
    ...
    while len(frontier) > 0:
        ...
        path_cost_till_now, path_till_now = pop_frontier(frontier)
        current_node = path_till_now[-1]
        path_cost_till_now = path_cost_till_now - get_geographical_heuristic_heuristic(current_node, goal)
        explored_nodes.append(current_node)
        # test goal condition
        if current_node == goal:
            return path_till_now, explored_nodes
        ...
        for neighbour in neighbours_list_str:
            ...
            extra_cost = 1
            neighbour_cost = extra_cost + path_cost_till_now + get_geographical_heuristic_heuristic(neighbour, goal)
            new_element = (neighbour_cost, path_to_neighbour)
...
def get_geographical_heuristic_heuristic(node, goal):
    i, j = divmod(int(node), 8)
    i_goal, j_goal = divmod(int(goal), 8)
    i_delta = abs(i - i_goal)
    j_delta = abs(j - j_goal)

    geographical_dist = i_delta + j_delta
